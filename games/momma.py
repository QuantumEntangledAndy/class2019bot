...
class Girl():
    ...
    def __init__(self, name):
        self.hobbies = ['exercise','meditation','read books','painting','gardening','watching television','shopping']
        self.field_of_studies = ['science','art','social science','bussiness','medical']
        self.careers = ['doctor','teacher','salesperson','researcher','artist']
        self.music_genres = ['pop','rock','jazz','R&B','classic']
        self.crusines = ['Thai','Japan crusine','Italian','Indian','Korean']
...
class Event():
    ...
    def common_value(self):      #To make the game harder by making girls to have something in common
    ...
    def ask_q(self,name):
        ...
        choices = self.choose(["Hobby", "Field of Study", "Favourite Crusine","Favourite Song Genre"])
        if self.hard_mode == True:
            answer_reveal = random.randint(0,1)
            if answer_reveal == 0:
                name.asked_Q +=1
                self.say(f"\n[The Boy] What is your {choices.lower()}?")
                self.say(f"[{name.name}] Why are you asking that? I don’t want to answer that question, sorry.")
                self.say("[The Boy] I... I'm sorry.\n")
                return 'Not answer'
        if choices == "Hobby" and name.q_check[0] == False:
            name.asked_Q +=1
            self.say(f"\n[The Boy] What is your {choices.lower()}?")
            ...
            name.q_check[0] = True
            return name.hobby
        elif choices == "Field of Study" and name.q_check[1] == False:
            name.asked_Q +=1
            self.say(f"\n[The Boy] What is your {choices.lower()}?")
        ...
        elif choices == "Favourite Crusine" and name.q_check[2] == False:
            name.asked_Q +=1
            self.say(f"\n[The Boy] What is your {choices.lower()}?")
            self.say(f"[{name.name}] I love {name.crusine} crusine! That's my favourite.")
            self.say("[The Boy] Thanks for the answer.\n")
            name.q_check[2] = True
            return name.crusine
        elif choices == "Favourite Song Genre" and name.q_check[3] == False:
            name.asked_Q +=1
            self.say(f"\n[The Boy] What is your {choices.lower()}?")
            self.say(f"[{name.name}] I like to listen to {name.music_genre} songs.")
            self.say("[The Boy] Thanks for the answer.\n")
            name.q_check[3] = True
    ...
    def cont(self,name,list_q):
        while True:
            ...
            if resume == 'End the investigation':
                self.say("\nAre you sure that you have all information that you needed?")
    ...
    def select_girl(self):
        ...
        while True:
            ...
            if choices == self.mom.name:
                ...
                if self.hard_mode == False or trial >= 1:
                    #print("you lose")
                    return 'LOSE'
...
class Story(Event):
    ...
    def intro(self):
        self.say("Once upon a time, in the land of sophisticated innovation, the boy accidentally walked into a time machine.")
        self.say("[The Boy] Oh! Awwww! My head is aching!! I’m about to throw up now! BBLLAARGGHH!!")
        self.say("[The Boy] Oh the machine stop spinning. Where am I now! Oh god damn its 1960!")
        self.say("He saw his dad and be able to recognize him. By the way. He was shocked that he can’t remember any of his mother’s appearance.")
        self.say("After wandering around his parent’s college, the boy saw his dad talking with a bunch of girls!")
        self.say("[The Boy] Oh my goodness! If he marries another woman, beside my mom. What should I do now!! I will not be born!!!")
        self.say("The boy cries as loud as rainstorm in the middle of the ocean, unknowing of his future that he will still stay alive in the time where he came from.")
        self.say("Players! We would like you to help us find his real mother, in order to let he be survived by investigating four of his father’s college girl.")
        self.say("Select one of the four that you think that is his real mother")
        self.say("GOOD LUCK. LET’S THE INVESTIGATION BEGINS!!\n")

    def mother_info(self,mom):
        self.say2("------------------------------")
        self.say("\n[The Boy] (thinking…) Now...what do I remember about my mother?")
        self.say("Wow... the time machine seems to have erased some of my memory... ")
        self.say("[The Boy] I can't even remember her name...")
        self.say("[The Boy] Let me jot down what I remember...\n")
        self.say2("------------")
        self.say(f"'My Mother'\n   Favorite Cuisine: {mom.crusine.title()}\n   Career: {mom.career.title()}\n   Favourite Music Genre: {mom.music_genre.title()}\n   Hobby: {mom.hobby.title()}")
        self.say2("------------")
    ...
    def direction(self):
        ...
        self.say("[Dad] My son… in order to solve the mystery of who is your real mother you will have to investigate four of my girlfriends.")
        self.say("[Dad] Their names are Sarah, Grace, Daisy and Nina.")
        self.say("[Dad] Good Luck!")
        self.say2(".\n.")
        self.say("--- Note: You are only allowed to ask two questions per girl. ---")
        self.say("--- Note: Different girls may possess similar characteristics. ---")
        self.say2(".\n.")

    def girl_talk(self,name):
        self.say(f"\n[The Boy] Excuse me...are you {name.name}?")
        self.say(f"[{name.name}] Yes...that is me.")
        self.say("[The Boy] I’m sorry to disturb you. Can I ask you a few questions?")
    ...
    def end_invest(self):
        ...
        self.say("...")
        self.say("[DAD] So?? Now that you have spoken to all the girls... who do you think is your real mother?\n")
    ...
    def good_end(self):
        ...
        self.say("Years ahead in the future...we find ourselves in a delivery room of a hospital.")
        self.say("After hours of labour...the anxious parents found peace in the sound of a baby wailing...")
        self.say("[BABY] Waah! Waaah!")
        self.say("[PARENTS] Oh my ...what a wonderful baby...he has done so much for us already.")
        self.say(" ")
        self.say("""
                ================
                Congratulations
    ...
    def bad_end(self):
        self.say("\nUnfortunately, that girl you chosen is not your real mother!")
        self.say("You realised that you have fail your most important decision.")
        self.say("And that is your last decision...")
        self.say("Your body is fading away... you don't know why...")
        self.say("You chose poorly, and you shall pay the price with your existence.")
    ...
    def play(self):
        ...
        while loop == "what_girl":
            ...
            while loop == "ask_q":
                ...
                if event.check_q(girl_name) == True:
                    answer = event.ask_q(girl_name)
                    loop = event.cont(girl_name,["Ask more question about her", "Ask someone else", "End the investigation"])

                else:
                    self.say("Sorry, you have already asked her 2 questions!")
                    self.say("You can't ask each girl more than 2 questions.")
                    loop = event.cont(girl_name,["Ask someone else", "End the investigation"])
